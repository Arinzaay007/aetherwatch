...
def _debug_opensky():
    ...
    from config.settings import OPENSKY_USERNAME, OPENSKY_PASSWORD, FORCE_MOCK_DATA

    lines = []
    lines.append(f"FORCE_MOCK_DATA  : {FORCE_MOCK_DATA}")
    lines.append(f"OPENSKY_USERNAME : '{OPENSKY_USERNAME}' (len={len(OPENSKY_USERNAME)})")
    lines.append(f"OPENSKY_PASSWORD : '{'*' * len(OPENSKY_PASSWORD)}' (len={len(OPENSKY_PASSWORD)})")

    # Test OpenSky
    try:
        auth = HTTPBasicAuth(OPENSKY_USERNAME, OPENSKY_PASSWORD) if OPENSKY_USERNAME else None
        r = requests.get(
            "https://opensky-network.org/api/states/all",
            params={"lamin": 45, "lomin": -10, "lamax": 55, "lomax": 20},
            auth=auth,
            timeout=15,
        )
        states = r.json().get("states") or []
        lines.append(f"OpenSky status  : {r.status_code}")
        lines.append(f"OpenSky aircraft: {len(states)}")
    except Exception as e:
        lines.append(f"OpenSky ERROR   : {e}")

    # Test adsb.fi fallback
    try:
        r2 = requests.get("https://opendata.adsb.fi/api/v2/aircraft", timeout=10)
        ac = r2.json().get("aircraft", [])
        lines.append(f"adsb.fi status  : {r2.status_code}, aircraft={len(ac)}")
    except Exception as e:
        lines.append(f"adsb.fi ERROR   : {e}")

    # Test Streamlit secrets
    try:
        secret_user = st.secrets.get("OPENSKY_USERNAME", "NOT FOUND")
        secret_pass = st.secrets.get("OPENSKY_PASSWORD", "NOT FOUND")
        lines.append(f"st.secrets user : '{secret_user}' (len={len(str(secret_user))})")
        lines.append(f"st.secrets pass : len={len(str(secret_pass))}")
    except Exception as e:
        lines.append(f"st.secrets ERROR: {e}")
...
def render_sidebar(all_cameras: list[dict]) -> dict:
    with st.sidebar:
        ...
        with st.expander("⚙️ System Info"):
            ...
            st.caption(f"**OpenSky user:** '{settings.OPENSKY_USERNAME}'")
...
def main():
    ...
    # ── DEBUG PANEL (remove once live data is confirmed working) ──────────────
    with st.expander("🔍 Debug — OpenSky Connection Test", expanded=False):
        if st.button("▶ Run Connection Test"):
            with st.spinner("Testing connections…"):
                result = _debug_opensky()
            st.code(result)
        else:
            st.caption("Click the button above to test OpenSky connectivity and credentials.")
